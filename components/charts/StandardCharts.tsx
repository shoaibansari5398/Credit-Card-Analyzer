  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-gray-200 p-2 shadow-lg rounded text-xs">
        <p className="font-semibold text-gray-800">{label || payload[0].payload.name}</p>
        <p className="text-blue-600">
          {payload[0].value ? `$${Number(payload[0].value).toFixed(2)}` : ''}
        </p>
        {payload[0].payload.merchant && (
             <p className="text-gray-500">{payload[0].payload.merchant}</p>
        )}
      </div>
    );
  }
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                ...
                <PolarAngleAxis dataKey="day" tick={{ fill: '#6B7280', fontSize: 10 }} />
        ...
        <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <XAxis 
                    ...
                    tick={{fontSize: 10}}
                />
                <YAxis dataKey="y" type="number" name="Amount" unit="$" tick={{fontSize: 10}}/>
