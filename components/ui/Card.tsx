    ...
    <div className={`bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex flex-col ${className}`}>
      {(title || action) && (
        <div className="flex justify-between items-center mb-4">
          {title && <h3 className="text-gray-500 text-sm font-semibold uppercase tracking-wide">{title}</h3>}
...
export const KPICard: React.FC<{ label: string; value: string; subValue?: string; trend?: 'up' | 'down' | 'neutral' }> = ({ label, value, subValue, trend }) => {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
      <h3 className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1">{label}</h3>
