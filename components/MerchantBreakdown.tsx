    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="text-center p-3 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Total Merchants</p>
          <p className="text-lg font-bold text-gray-900">{merchants.length}</p>
        </div>
        <div className="text-center p-3 bg-emerald-50 rounded-lg">
          <p className="text-xs text-emerald-600 uppercase tracking-wide mb-1">Top Merchant</p>
          <p className="text-sm font-bold text-emerald-800 truncate">{merchants[0]?.name || '-'}</p>
        </div>
        <div className="text-center p-3 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Top % Share</p>
          <p className="text-lg font-bold text-gray-900">{merchants[0]?.percentage.toFixed(1) || 0}%</p>
        </div>
      </div>
          ...
          {displayMerchants.map((merchant, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                    i < 3 ? 'bg-amber-100 text-amber-700' : 'bg-gray-200 text-gray-600'
                  }`}
                >
                  {i + 1}
                </span>
                <div>
                  ...
                  <p className="text-xs text-gray-500">{merchant.category} • {merchant.count} transactions</p>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">{merchant.percentage.toFixed(1)}%</p>
              </div>
            </div>
          ))}
        </div>
